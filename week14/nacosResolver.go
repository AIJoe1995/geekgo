...
func (r *nacosResolver) ResolveNow(options resolver.ResolveNowOptions) {
	...
	fmt.Println("r.target.Endpoint() ", r.target.Endpoint()) //  user
	fmt.Println("r.target.String() ", r.target.String())     // nacos:///user
	fmt.Printf("r.target.URL %+v", r.target.URL)             //   {nacos    /user  false false   }
